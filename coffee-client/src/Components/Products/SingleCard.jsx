...
const SingleCard = ({ coffee, coffeeData, setCoffeeData }) => {
    let handleDelete = (id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`https://cofee-server.vercel.app/coffees/${id}`, {
                    method: 'DELETE',
                })
                    .then(res => res.json())
                    .then(data => {
                        if (data.deletedCount > 0) {
                            ...
                            let remainingCard = coffeeData.filter(card => card._id !== coffee._id);
    ...
    return (
        <div className='bg-[#F5F4F1] px-20 rounded-md py-4 flex gap-6 items-center justify-between'>
            <div>
                <img className='w-[125px] h-[180px]' src={coffee.imgUrl} alt="" />
            </div>
            <div>
                <p className='text-xl'>Name: <span className='text-[#5C5B5B]'>{coffee.name}</span></p>
                <p className='text-xl'>Chef: <span className='text-[#5C5B5B]'>{coffee.chef}</span></p>
                <p className='text-xl'>Price: <span className='text-[#5C5B5B]'>{coffee.price}</span></p>
            </div>
            <div className='flex flex-col gap-3'>
                <Link to={`/coffeeDetails/${coffee._id}`}>
                    <button className='bg-[#D2B48C] p-2 rounded-md'>
                ...
                <button onClick={() => handleDelete(coffee._id)} className='bg-[#EA4744] p-2 rounded-md'>
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
