...
const SingleCard = ({ coffee }) => {
    let { name, chef, price, imgUrl } = coffee;
    return (
        <div className='bg-[#F5F4F1] px-20 rounded-md py-4 flex gap-6 items-center justify-between'>
            <div>
                <img className='w-[125px] h-[180px]' src={imgUrl} alt="" />
            </div>
            <div>
                <p className='text-xl'>Name: <span className='text-[#5C5B5B]'>{name}</span></p>
                <p className='text-xl'>Chef: <span className='text-[#5C5B5B]'>{chef}</span></p>
                <p className='text-xl'>Price: <span className='text-[#5C5B5B]'>{price}</span></p>
            </div>
